...
extends Spatial
class_name Feature_UIRigidBodyGrabRayCast

# The controller that this grab feature is bound to
var controller : ARVRController = null;
var raycast : Spatial = null;
...
var other_grab_feature : Feature_UIRigidBodyGrabRayCast = null
		...
		if controller.controller_id == 1:# left
			if vr.rightController:
				for c in vr.rightController.get_children():
					...
					if c.get_class() == "Feature_UIRigidBodyGrabRayCast":
