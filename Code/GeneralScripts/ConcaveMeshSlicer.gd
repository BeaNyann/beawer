				...
				for j in range(len(polygons)):
					...
					if j != i:
						...
						for k in range(len(polygon_check)):
							...
							var point_segment = Geometry.get_closest_point_to_segment(point,pos2d,pos2d2)
							if point_segment.distance_to(point) < closest_dist:
								closest_dist = point_segment.distance_to(point)
								closest_point = point_segment
							
						if Geometry.is_point_in_polygon(point, Array(polygon2d)) == true:
							is_inside = true
...
func _to_transform_local(ttransform: Transform, global_pos: Vector3) -> Vector3:
	var xform := ttransform.affine_inverse()
	return Vector3(
...
func _to_transform_local2(ttransform: Transform, global_pos: Vector3) -> Vector3:
	var xform := ttransform.affine_inverse()
	return Vector3(
		ttransform.basis[0].dot(global_pos) + xform.origin.x ,
		ttransform.basis[1].dot(global_pos) + xform.origin.y,
		ttransform.basis[2].dot(global_pos) + xform.origin.z
	)
